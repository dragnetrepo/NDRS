...
const ProfileSetup2 = () => {
  ...
  const fetchUnionBranches = async () => {
    try {
      ...
      document.getElementById(
        "branches"
      ).innerHTML = `<option value="" disabled selected>Fetching Union Branches...</option>`;
      document.getElementById("organizations").innerHTML = ``;

      if (selected_union) {
        const res = await fetch(
          baseUrl + `/api/get-union-branches/${selected_union}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const fetchUnionSubBranches = async () => {
    try {
      ...
      document.getElementById(
        "organizations"
      ).innerHTML = `<option value="" disabled selected>Fetching Sub Branches...</option>`;

      if (selected_union_branch) {
        const res = await fetch(
          baseUrl + `/api/get-union-organizations/${selected_union_branch}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="mb-4">
                    ...
                    <select
                      ...
                      id="branches"
                      name="union_branch"
                      onChange={fetchUnionSubBranches}
                    ></select>
                  </div>
