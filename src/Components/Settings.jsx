...
const Settings = () => {
	...
	const [getOrgnizations, setOrganizations] = useState([]);
	...
	const getAllOrganizations = async (result) => {
		try {
			if (!allowedRoleChangeUsers.includes(result.user_role?.role_name)) {
				const token = localStorage.getItem("token");
				...
				if (result.user_role?.union_sub_branch_name) {
					organization_url = `union/organizations`;
				}
				else if (result.user_role?.union_branch_name) {
					organization_url = `union/branch`;
				}
				else if (result.user_role?.union_name) {
					organization_url = `get-unions`;
				}

				console.log(organization_url);

				const res = await fetch(baseUrl + `/api/${organization_url}`, {
					headers: {
						Authorization: `Bearer ${token}`,
					},
				});
	...
	const handleSetNewRoleSelected = (id) => {
		setNewRole({
			role_id: id
		});
	}

	const submiteNewRoleRequest  = async () => {
		setisSendingRequest(true);

		try {
			const res = await fetch(baseUrl + "/api/users/requests/new-role", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${localStorage.getItem("token")}`,
			...
			document.getElementById("request-new-role-close-modal").click();
			handleSetNewRoleSelected(null);
		} catch (error) {
			console.error("Error updating profile:", error.message);
		}
		finally {
	...
	const handleSetNewOrg = (e) => {
		setNewOrg({
			org_id: e.target.value
		});
	}

	const submiteNewOrganzationRequest  = async (e) => {
		e.preventDefault();
		setisSendingRequest(true);

		try {
			const res = await fetch(baseUrl + "/api/users/requests/new-organization", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${localStorage.getItem("token")}`,
			...
			toast.success(data.message);
			document.getElementById("close-current-org-modal").click();
			setNewOrg({
				org_id: null
			});
		} catch (error) {
	...
	const handldeleteOrg = (e) => {
		setDeleteOrg({
			reason: e.target.value
		});
	}

	const submitDeleteOrganzationRequest  = async (e) => {
		e.preventDefault();
		setisSendingRequest(true);

		try {
			const res = await fetch(baseUrl + "/api/users/requests/delete-organization", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					Authorization: `Bearer ${localStorage.getItem("token")}`,
				},
				body: JSON.stringify(deleteOrg),
			});
			...
			toast.success(data.message);
			document.getElementById("delete-organization-modal-close").click();
			setNewOrg({
				reason: null
			});
		} catch (error) {
		...
		<>
																				...
																				<div className="row mt-4">
																					<div className="col-lg-3 mb-lg-0 mb-4">
																						...
																						<div className="main-avatar mx-auto">
																							<img
																							src={user.display_picture || avatarImage}
																					...
																					<div className="col-lg-9">
																						...
																						{!(allowedRoleChangeUsers.includes(user.user_role?.role_name)) && (
																							<div className="mb-4">
																								<a href="#" className="text-main-primary" data-bs-toggle="modal" data-bs-target="#orgModal">
																	...
																	<div className="card-body p-4">
																		<div className="d-flex justify-content-between align-items-center flex-wrap">
																			<div className="text-start mb-lg-0 mb-3">
																				...
																				<p className="mb-0">
																					Delete your account from the NDRS database?
																				</p>
																			</div>

																			<button className="btn btn-outline-danger btn-size" data-bs-toggle="modal" data-bs-target="#deleteModal">
																				Delete account
																			</button>
																		</div>
																	</div>
															...
															<div className="col-lg-8">
																<div className="card mb-4">
																	<div className="card-header p-4 heading-card bg-white d-flex align-items-center justify-content-between flex-wrap">
																		...
																		<div className="d-flex align-items-center gap-15">
																			<a href="" className="btn-flat text-main-primary">
																				Discard changes
																			</a>
																...
																<div className="card delete-card">
																	<div className="card-body p-4">
																		<div className="d-flex justify-content-between align-items-center flex-wrap">
																			...
																			<button className="btn btn-outline-danger btn-size" data-bs-toggle="modal" data-bs-target="#deleteModal2">
																				Delete account
																			</button>
																		</div>
																	</div>
															...
															<div className="card card-box-view mb-4">
																...
																<div className="d-flex justify-content-between align-items-center flex-lg-nowrap flex-wrap">
																	...
																	<input
																		...
																		checked={twoFactorAuth.setting}
																		name="setting"
																		onChange={twoFactorAuthentification}
																	/>
																	<label
				...
				<div className="modal-dialog modal-dialog-centered">
					<div className="modal-content border-0 p-lg-4 p-3">
						<div className="text-end">
							<button type="button" id="request-new-role-close-modal" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
						</div>
						<div className="modal-body">
							...
							<p className="mb-4 modal-text text-center">
								Current role - {user.user_role?.role_name}
							</p>
							...
							<form>
								...
								<ul className="list-group new-role">
									{getAdminRoles.length && getAdminRoles.map(item => (
										<li className={`list-group-item d-flex justify-content-between cursor-pointer ${newRole.role_id === item._id ? `active` : ``} p-3`} onClick={(e) => handleSetNewRoleSelected(item._id)}>
											{item.name}
											{newRole.role_id === item._id && (
								...
								<div className="mt-4">
									<button className="btn btn-size btn-main-primary w-100" onClick={submiteNewRoleRequest} disabled={isSendingRequest}>
			...
			<div className="modal fade" id="orgModal" tabIndex="-1" aria-labelledby="orgModalLabel" aria-hidden="true">
				<div className="modal-dialog modal-dialog-centered">
					<div className="modal-content border-0 p-lg-4 p-3">
						<div className="text-end">
							<button type="button" id="close-current-org-modal" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
						</div>
						<div className="modal-body">
							<div className="text-center">
								<img
							...
							<p className="mb-4 modal-text text-center">
								...
								{(user.user_role?.union_branch_name && !user.user_role?.union_sub_branch_name) ? (
									user.user_role?.union_branch_name
								) : null}

								{(user.user_role?.union_name && !user.user_role?.union_sub_branch_name && !user.user_role?.union_branch_name) ? (
									user.user_role?.union_name
								) : null}
							</p>

							<form onSubmit={(e) => {submiteNewOrganzationRequest(e)}}>
								<div className="mb-4">
									<label className="form-label">Current organization</label>
									<select className="form-control form-control-height" defaultValue="" onChange={(e) => {handleSetNewOrg(e)}} required>
										<option value="" selected disabled>--Choose--</option>

										{getOrgnizations.length && getOrgnizations.map(item => (
											<option selected={item._id===newOrg.org_id} value={item._id}>{item.name}</option>
										))}
									</select>
								</div>
								<div className="">
									<button className="btn btn-size btn-main-primary w-100" disabled={isSendingRequest}>
										{isSendingRequest ? `Sending Request...` : `Send Request`}
									</button>
								</div>
							</form>
			...
			>
				...
				<div className="modal-content border-0 p-lg-4 p-3">
					<div className="text-end">
					<button type="button" id="delete-organization-modal-close" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
					</div>
					<div className="modal-body">
						<div className="text-center">
							<img
						...
						<h1 className="heading modal-heading text-center mb-3">
							Why are you requesting to delete 
							“{user.user_role?.union_sub_branch_name ? (
								user.user_role?.union_sub_branch_name
							) : null}
						...
						<form onSubmit={(e) => {submitDeleteOrganzationRequest(e)}}>
							<div className="mb-4">
								<label className="form-label">Reason for request</label>
								<textarea onChange={(e) => {handldeleteOrg(e)}} className="form-control" rows="4" placeholder="Type your reason" required></textarea>
							</div>
							<div className="">
								<button className="btn btn-size btn-main-danger w-100" disabled={isSendingRequest}>
									{isSendingRequest ? `Sending Delete Request...` : `Send Delete Request`}
								</button>
							</div>
						</form>
					</div>
					...
					<form action="" method="post" onSubmit={handleSubmitPassword}>
						...
						<div className="input-group">
							...
							className={`form-control border-end-0 form-control-height ${
								...
								changePassword.password_confirmation && `is-invalid`
							}`}
