        ...
        if ($union_branches->isNotEmpty()) {
            foreach ($union_branches as $union_branch) {
                ...
                $data[] = [
                    ...
                    "logo" => $union_branch->logo ? asset('/union_branch/'.$union_branch->logo) : '',
                ...
                if ($role) {
                    if ($user) {
                        ...
                        if (UnionUserRole::where("user_id", $user->id)->where("union_id", $union_branch->union_id)->exists()){
                            ...
                            send_outgoing_email_invite($request->email, "simple-invite", $union_branch->name, $role->name);
                        }
                    }
                    else {
                        if (EmailInvitations::where("email", $request->email)->where("union_id", $union_branch->id)->exists()) {
                            ...
                            send_outgoing_email_invite($request->email, "invite-with-link", $union_branch->name, $role->name, $url_token);
                        }
                    }
                }
                else {
